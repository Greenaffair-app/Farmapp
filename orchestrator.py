...
def _query_soilgrids_wcs_point(lat: float, lon: float) -> dict:
    """FALLBACK ONLY -- used when the primary REST API (rest.isric.org)
    fails, e.g. during ISRIC's announced indefinite REST API outage
    (August 2026). Uses a completely separate service (maps.isric.org,
    the WCS/Web Coverage Service) that may still be operational.

    IMPORTANT CAVEATS, stated honestly:
    - This has NOT been tested against the live service. The sandbox
      this was written in cannot reach maps.isric.org at all (not on
      its network allowlist), so this is a best-effort implementation
      based on ISRIC's official documentation, not a verified one.
    - SoilGrids' native coordinate system is NOT plain lat/lon -- it's
      an Interrupted Goode Homolosine projection (EPSG:152160). Every
      point must be transformed into that projection before querying,
      or the result will silently point at the wrong location.
    - Conversion factors below ("mapped" raw integer -> real-world
      units) are taken from ISRIC's published SoilGrids specification.
      Unlike the REST function above, WCS doesn't return these factors
      alongside the data, so they're hardcoded here -- verify against
      https://docs.isric.org/globaldata/soilgrids/SoilGrids_faqs.html
      if any value looks implausible after real testing.
    - Requires the `soilgrids`, `rasterio`, and `pyproj` packages --
      these pull in GDAL, a heavy system-level geospatial library.
      This may or may not install cleanly on Render (the same category
      of risk that caused the Playwright/Chromium deployment issues) --
      needs real deployment testing to confirm.
    """
    from pyproj import Transformer
    from soilgrids import SoilGrids

    # WGS84 (plain lat/lon) -> SoilGrids' native Homolosine projection
    transformer = Transformer.from_crs("EPSG:4326", "EPSG:152160", always_xy=True)
    x, y = transformer.transform(lon, lat)

    # Tiny box around the point -- one pixel's worth at 250m resolution
    d = 150  # meters
    west, east = x - d, x + d
    south, north = y - d, y + d

    # property_name: (WCS service_id, coverage_id, conversion factor to divide by)
    properties = {
        "ph":                    ("phh2o",    "phh2o_0-5cm_mean",    10),
        "organic_carbon_g_per_kg": ("soc",    "soc_0-5cm_mean",      10),
        "clay_pct":              ("clay",     "clay_0-5cm_mean",     10),
        "sand_pct":              ("sand",     "sand_0-5cm_mean",     10),
        "silt_pct":              ("silt",     "silt_0-5cm_mean",     10),
        "nitrogen_g_per_kg":     ("nitrogen", "nitrogen_0-5cm_mean", 100),
        "cec_cmol_per_kg":       ("cec",      "cec_0-5cm_mean",      10),
        "bulk_density_kg_dm3":   ("bdod",     "bdod_0-5cm_mean",     100),
        "water_retention_pct":   ("wv0033",   "wv0033_0-5cm_mean",   10),
    }

    sg = SoilGrids()
    values = {}
    for key, (service_id, coverage_id, factor) in properties.items():
        try:
            data = sg.get_coverage_data(
                service_id=service_id, coverage_id=coverage_id,
                west=west, south=south, east=east, north=north,
                crs="urn:ogc:def:crs:EPSG::152160",
                width=1, height=1,
            )
            raw_val = float(data.values.flatten()[0])
            values[key] = round(raw_val / factor, 2) if raw_val is not None else None
        except Exception:
            values[key] = None
...
def get_soil_data(lat: float, lon: float) -> dict:
    ...
    If the primary REST API fails entirely (e.g. connection/server error,
    not just "no data at this pixel"), falls back to the WCS-based method
    above before giving up -- see that function's docstring for real
    caveats about this fallback's untested status.
    """
    ...
    try:
        ...
        # REST API failed entirely (not just missing data) -- try the
        # WCS fallback before giving up
        try:
            values = _query_soilgrids_wcs_point(lat, lon)
            if any(v is not None for v in values.values()):
                return {"source": "ISRIC SoilGrids (WCS fallback)", "status": "ok", **values}
            return {"source": "ISRIC SoilGrids", "status": "error", "error": str(rest_error)}
        except Exception as wcs_error:
            return {
                "source": "ISRIC SoilGrids", "status": "error",
                "error": f"REST failed ({rest_error}); WCS fallback also failed ({wcs_error})",
            }
